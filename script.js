...
document.addEventListener('DOMContentLoaded', function() {
  ...
  if (contactForm) {
    contactForm.addEventListener('submit', function(e) {
      ...
      if (!window.zappyContactFormLoaded) {
        console.log('⚠️ Zappy: Handler not properly initialized');
      } else {
        (async function() {
          try {
            ...
            const response = await fetch('https://api.zappy5.com/api/email/contact-form', {
              ...
              body: JSON.stringify({
                websiteId: 'd30c0cf6-14f2-4943-bd90-be892e85b44b',
                name: data.name || '',
                email: data.email || '',
    ...
    "translations": {
      "he": {
        "consentModal": {
          ...
          "description": "אינדיס בע\"מ משתמש בעוגיות כדי לשפר את החוויה שלך, לנתח שימוש באתר ולסייע במאמצי השיווק שלנו.",
